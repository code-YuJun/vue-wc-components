...
4.3 / 4.4
## 你们实际用 webcomponent 组件库做了什么组件
## 为什么打包的产物都是一个 JS 和 一个 CSS 呢，不多也不少
组件需要一次性加载，而且拆分成多个文件之后， loader.js 加载资源的时候不能固定创建 link 和 script 标签的数量，所以组件库需要统一打包产物的输出。
## npm 发包
- npm login
- npm publish
...
## 为什么需要有配置系统
组件发布到了 npm 之后，loader.js 需要拿到组件最新的元信息（名称、版本号），从而获取最新的组件内容，所以此时需要 loader.js 去调用一个配置系统的接口。发布 npm 包之后，需要将及时将组件的信息同步到配置系统中。这样 loader.js 就能拿到最新的组件信息
## 配置系统如何设计
组件的元信息（组件名称、版本号）
降级开关
环境（预发布环境、线上环境）
...
loader.js 不应该缓存，如果 loader.js 中的内容有调整，如果有缓存则可能会出问题，loader.js 的风险较高
组件资源 跟版本走的，有问题的时候，随时切换版本，每一个版本都在 npm 库里，所以需要给每一个版本作缓存
